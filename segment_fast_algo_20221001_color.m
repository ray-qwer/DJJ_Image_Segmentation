...
for m = 1:sz(1)
    for n = 1:sz(2)
        ...
        if n > 1
            left_region = R(m,n-1);
            leftdiff = mean(pixel - m_n_list(left_region,1:3));  gradLeftDiff = abs(leftdiff)+abs(squeeze(ghrgb(m,n,:)))*lambda;
            if gradLeftDiff <= th
                col_combine = true;
            end
        end
        if m > 1
            up_region = R(m-1,n);
            updiff = mean(pixel - m_n_list(up_region,1:3)); gradUpDiff = abs(updiff)+abs(squeeze(gvrgb(m,n,:)))*lambda;
            if gradUpDiff <= th
                row_combine = true;
            end
        end
